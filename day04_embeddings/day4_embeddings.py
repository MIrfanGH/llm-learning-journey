...
def cosine_similarity(vec_a, vec_b): 
    ...
    return np.dot(vec_a, vec_b) / (np.linalg.norm(vec_a) *np.linalg.norm(vec_b)) # it returns the cosine similarity score between two vectors, which ranges from -1 to 1. 
...
def search(query, doc_embeddings, sentences, top_k=5):
    ...
    """
    We sort the list of (sentence, score) tuples to rank documents by relevance.
    Without sorting, we’d just take the first 5 documents in file order, not the most relevant ones.
    The 'key' ensures sorting is based on the score (second element of the tuple).
    'reverse=True' makes the ranking descending, so the highest scores (most relevant documents) appear first.
    """
...
def main():
    ...
    for i, (sentence, score) in enumerate(results, start=1): 
        # it’s just like looping with i, num when we have a list of numbers — but here, 
        # since each item is a tuple, we unpack it into sentence and score.

        print(f"{i}. Score: {score:.4f}") # The score is formatted to 4 decimal places for better readability.
        print(f"   {sentence}\n")
